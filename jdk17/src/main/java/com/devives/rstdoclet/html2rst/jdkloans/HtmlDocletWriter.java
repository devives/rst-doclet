...
public class HtmlDocletWriter {
    ...
    public final Element parentDoc;
...
//    }
     ...
     */
    public InlineElement getPackageLink(PackageElement packageElement, Content label) {
...
//            }
     ...
     */
    private static InlineElement hrefToLink(String a) {
        ...
        if (relativeLinkLowerCase.startsWith("mailto:") ||
            ...
            return Rst4Sphinx.elements().link(href, StringUtils.findFirstNotNullOrEmpty(text, href));
        } else {
     ...
     */
    public InlineElement getModuleLink(ModuleElement mdle, Content label) {
        ...
        if (flags.contains(ElementFlag.PREVIEW)) {
            ...
            return Rst4Sphinx.elements().text(link.toString());
        }
        return Rst4Sphinx.elements().text(label.toString());
    }
//
...
//        HtmlLinkInfo LinkInfoImpl = new HtmlLinkInfo(configuration, context, (TypeElement) element);
...
//    }
    ...
    public InlineElement seeTagToContent(Element element, DocTree see, TagletWriterImpl.Context context) {
        ...
        switch (kind) {
            ...
            case SEE -> {
                ...
                switch (ref.get(0).getKind()) {
                    case TEXT -> {
                        // @see "Reference"
                        return Rst4Sphinx.elements().text("\\ " + StringUtils.dequote(seeText, '\"') + "\\ ");
                    }
                    case START_ELEMENT -> {
                        // @see <a href="...">...</a>
                        return hrefToLink(replaceDocRootDir(removeTrailingSlash(seeText)));
                    }
                    case REFERENCE -> {
                        // @see reference label...
                        label = ref.subList(1, ref.size());
                    }
                    default -> throw new IllegalStateException(ref.get(0).getKind().toString());
                }
            }
        ...
        if (refClass == null) {
            ...
            if (refPackage != null && utils.isIncluded(refPackage)) {
                ...
                if (elementCrossLink != null) {
                    ...
                    Content link = links.createExternalLink(elementCrossLink,
                            (labelContent.isEmpty() ? text : labelContent));
                    return hrefToLink(link.toString());
                } else {
                    // No cross link found so print warning
                    DocTreePath docTrees = ch.getDocTreePath(see);
                    if (docTrees != null) {
                        messages.warning(docTrees,
                                "doclet.see.class_or_package_not_found",
                                "@" + tagName,
                                seeText);
                    } else {
                        messages.warning("doclet.see.class_or_package_not_found",
                                "@" + tagName,
                                seeText);
                    }
                    return Rst4Sphinx.elements().text((labelContent.isEmpty() ? text : labelContent).toString());
                }
            }
...
//                    .label(labelContent));
            ...
            if (ch.getText(see).trim().startsWith("#") &&
                ...
                if (parentDoc instanceof TypeElement typeElement) {
                    containing = typeElement;
...
//            Content link = getDocLink(HtmlLinkInfo.Kind.SEE_TAG, containing,
...
//                            : labelContent), null, false);
    ...
    private Content plainOrCode(boolean plain, Content body) {
        ...
        return (plain || body.isEmpty()) ? body : body;
    }
//
//    /**
...
//    }
    ...
    private boolean shouldRedirectRelativeLinks(Element element) {
        ...
        if (currentPageElement == null) {
            if (parentDoc instanceof PackageElement packageElement) {
                currentPageElement = packageElement;
            } else if (parentDoc instanceof ModuleElement moduleElement) {
                currentPageElement = moduleElement;
            }
        }
    ...
    private boolean inSamePackage(Element element) {
        Element currentPageElement = (this instanceof PackageElement packageElement)
                ? packageElement : getCurrentPageElement();
...
//                case CONSTRUCTOR -> ((TypeElement) forWhat.getEnclosingElement()).getSimpleName();
...
//    }
     ...
     *                 added
     */
    public void addReceiverAnnotationInfo(ExecutableElement method, List<? extends AnnotationMirror> descList,
                                          Content htmltree) {
     ...
     */
    public boolean addAnnotationInfo(int indent, Element doc, VariableElement param,
                                     Content tree) {
        return addAnnotationInfo(indent, doc, param.getAnnotationMirrors(), false, tree);
    }
     ...
     * @param doc      the doc to write annotations for.
     * @param descList the array of {@link AnnotationMirror}.
     * @param htmltree the documentation tree to which the annotation info will be
     *                 added
     */
    private boolean addAnnotationInfo(int indent, Element doc,
    ...
    public Content getSpace() {
        return com.devives.rstdoclet.html2rst.jdkloans.RawHtml.nbsp;
    }
