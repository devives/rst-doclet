...
import static com.devives.rstdoclet.html2rst.HtmlUtils.unescapeLtRtAmpBSlash;
...
public class RstDocumentWriter implements HtmlVisitor {
    ...
    public void visitLink(String href, String label) {
        if (href.startsWith("#")) {
            ...
            getTextBuilder().addChild(new Ref(href.substring(1), unescapeBrackets(label)));
            //getTextBuilder().role("ref", href.substring(1), unescapeBrackets(label));
        } else if (href.startsWith("@")) {
            getTextBuilder().addChild(new JavaRef(href.substring(1), unescapeBrackets(label)));
    ...
    private <PARENT extends RstNodeBuilder<?, ?, ?, ?>,
    ...
    private <BUILDER extends RstNodeBuilder<?, ?, ?, ?>> void endUntil(Class<BUILDER> builderClass) {
    ...
    private String unescapeBrackets(String text) {
