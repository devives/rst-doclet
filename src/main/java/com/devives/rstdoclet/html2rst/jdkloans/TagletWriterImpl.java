...
public class TagletWriterImpl extends TagletWriter {
    ...
    private final DocContext htmlWriter;
    private final ConfigurationImpl configuration;

    public TagletWriterImpl(DocContext htmlWriter, boolean isFirstSentence) {
    ...
    public Content seeTagOutput(Doc holder, SeeTag[] seeTags) {
        ...
        if (holder.isField() && ((FieldDoc) holder).constantValue() != null &&
                htmlWriter.getParentDoc() instanceof ClassDoc) {
            //Automatically add link to constant values page for constant fields.
            appendSeparatorIfNotEmpty(body);
            DocPath constantsPath =
                    htmlWriter.pathToRoot.resolve(DocPaths.CONSTANT_VALUES);
            String whichConstant =
                    ((ClassDoc) htmlWriter.getParentDoc()).qualifiedName() + "." + ((FieldDoc) holder).name();
