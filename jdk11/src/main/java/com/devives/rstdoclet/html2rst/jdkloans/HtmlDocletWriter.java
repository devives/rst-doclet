...
//    HtmlTree fixedNavDiv = new HtmlTree(HtmlTag.DIV);
     ...
     *
     * @param parentDoc     the element for witch documentation is generating
     * @param configuration the configuration for this doclet
     */
    public HtmlDocletWriter(Element parentDoc, HtmlConfiguration configuration) {
        if (parentDoc instanceof PackageElement) {
            this.path = new DocPaths(configuration.utils, false).forPackage((PackageElement)parentDoc);
        } else if (parentDoc instanceof ExecutableElement) {
            this.path = new DocPaths(configuration.utils, false).forClass((TypeElement) parentDoc.getEnclosingElement());
        } else if (parentDoc instanceof VariableElement) {
            this.path = new DocPaths(configuration.utils, false).forClass((TypeElement) parentDoc.getEnclosingElement());
        } else {
            this.path = new DocPaths(configuration.utils, false).forClass((TypeElement) parentDoc);
        }
...
//    }
    ...
    public InlineElement seeTagToContent(Element element, DocTree see) {

        Kind kind = see.getKind();
        if (!(kind == LINK || kind == SEE || kind == LINK_PLAIN)) {
            return Rst.elements().text("");
        }
        ...
        if (refClass == null) {
            ...
            if (refPackage != null && utils.isIncluded(refPackage)) {
                ...
                if (elementCrossLink != null) {
                    ...
                    Content link = links.createLink(elementCrossLink,
                    ...
                    messages.warning(ch.getDocTreePath(see),
                            ...
                            "@" + tagName,
                            seetext);
                    return Rst4Sphinx.elements().text((label.isEmpty() ? text : label).toString());
                }
            }
            ...
            if (ch.getText(see).trim().startsWith("#") &&
                ...
                if (parentDoc instanceof TypeElement) {
                    containing = (TypeElement) parentDoc;
...
//    }
    ...
    boolean ignoreNonInlineTag(DocTree dtree) {
        ...
        if (name != null) {
            ...
            if (htmlTag != null &&
                    htmlTag.blockType != HtmlTag.BlockType.INLINE) {
                return true;
            }
        }
        return false;
